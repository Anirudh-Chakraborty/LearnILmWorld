             ...
             {faqs.map((faq, index) => (
               ...
               >
                 <button onClick={() => toggleAccordion(index)} className="w-full px-6 py-4 text-left flex justify-between items-center focus:outline-none">
                   ...
                   {activeAccordion === index ? <ChevronUp className="w-5 h-5 p-1 bg-blue-200  rounded-full " /> : <ChevronDown className="bg-blue-200 w-5 h-5 rounded-full p-1 " />}
                 </button>
                 <div className={`px-6 text-gray-500 transition-all duration-300 ease-in-out overflow-hidden ${activeAccordion === index ? "max-h-40 py-4 pb-6 bg-blue-200" : "max-h-0"}`}>
                   {faq.a}
                 </div>
               </motion.div>
             ))}
